...
```bash
pipx install --force "plugin-scanner==3.0.99"
plugin-scanner scan . --format text
```

Expected reviewed wheel SHA256: `edff712545ccd5c16f0aabcda8c1112f7a129bd7507954cfaaea6a4c5f31ec89`

If you want to verify the exact wheel before install:

```bash
rm -rf .hol-plugin-scanner-dist
python3 -m pip download --only-binary=:all: --no-deps --dest .hol-plugin-scanner-dist "plugin-scanner==3.0.99"
